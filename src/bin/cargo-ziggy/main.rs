...
fn main() -> Result<(), anyhow::Error> {
	...
	match command {
		...
		Ziggy::Cover(mut args) => args.generate_coverage().context("Failure generating coverage"),
		...
		Ziggy::Triage(mut args) => {
			args.triage().context("Triaging with casr failed, try \"cargo install casr\"")
		},
	}
}
