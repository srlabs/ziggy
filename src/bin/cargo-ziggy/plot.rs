...
impl Plot {
	pub fn generate_plot(&mut self) -> Result<(), anyhow::Error> {
		...
		let fuzzer_data_dir =
			format!("{}/{}/afl/{}/", &self.ziggy_output.display(), &self.target, &self.input);
		...
		match Self::plot_corpus(
			&dir_path,
			&PathBuf::from(format!("{}/{}/", &self.ziggy_output.display(), &self.target)),
		) {
			Ok(_) => {},
			Err(e) => println!("Couldn't plot the corpus count : {:?} with {:?}", e, dir_path),
		}
	...
	pub fn plot_corpus(
		...
		let x: Vec<String> =
			data.iter().map(|(dt, _)| dt.format("%Y-%m-%d %H:%M:%S").to_string()).collect();
		let y: Vec<usize> = data.iter().map(|(_, count)| *count).collect();

		let trace = Scatter::new(x, y).name("Edge Count").mode(plotly::common::Mode::LinesMarkers);
		...
		println!("Corpus Count tracker has been saved as in {}", output_path.display());
