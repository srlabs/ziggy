...
fn get_target(target: String) -> Result<String> {
    ...
    fn get_new_target() -> Result<String> {
        let cargo_toml_string = fs::read_to_string("Cargo.toml")
            .context("⚠️  could not read the Cargo.toml for target guessing")?;
        let cargo_toml = cargo_toml_string
            .parse::<toml::Value>()
            .context("⚠️  could not parse the Cargo.toml for target guessing")?;
...
fn build_fuzzers(no_afl: bool, no_honggfuzz: bool) -> Result<(), anyhow::Error> {
    ...
    if !no_afl {
        ...
        let run = process::Command::new(cargo.clone())
            ...
            .env("AFL_QUIET", "1")
            .spawn()
            .context("⚠️  could spawn runner child")?
            .wait()
...
fn kill_subprocesses_recursively(pid: &str) -> Result<(), anyhow::Error> {
    ...
    let subprocesses = process::Command::new("pgrep")
        .arg(&format!("-P{pid}"))
        .output()
        .context("⚠️  could not pgrep for given pid")?;

    for subprocess in std::str::from_utf8(&subprocesses.stdout)
        .context("⚠️  could not parse pgrep stdout as utf8")?
        .split('\n')
    {
...
fn stop_fuzzers(processes: &mut Vec<process::Child>) -> Result<(), anyhow::Error> {
    ...
    for process in processes {
        ...
        process.kill().context(format!(
            "⚠️  could not kill process with pid {}",
            &process.id().to_string()
        ))?;
        process.wait().context(format!(
            "⚠️  could not wait for process with pid {}",
            &process.id().to_string()
        ))?;
    }
    Ok(())
...
fn run_fuzzers(args: &Fuzz) -> Result<(), anyhow::Error> {
    ...
    socket
        ...
        .context("⚠️  could not set statsd socket to non blocking")?;
    let mut buf = [0; 4096];
    ...
    loop {
        ...
        if let Ok((amt, _)) = socket.recv_from(&mut buf) {
            ...
            for msg in String::from_utf8(v)
                .context("⚠️  could not parse data from socket to utf8")?
                .split_terminator('\n')
            {
            ...
            term.move_cursor_up(11)
                .context("⚠️  could not move up cursor in terminal")?;
        ...
        if !execs_per_sec.is_empty() {
            ...
            if let (Ok(afl_crashes), Ok(honggfuzz_crashes)) = (
                ...
                for crash_input in afl_crashes.chain(honggfuzz_crashes).flatten() {
                    ...
                    fs::copy(crash_input.path(), to_path.clone()).context(format!(
                        "⚠️  could not copy the crash input {} to path {}",
                        crash_input.path().display(),
                        to_path.clone().display()
                    ))?;
                }
            }
        }
            ...
            match minimize_corpus(
                ...
                Ok(_) => {
                    ...
                    term.move_cursor_up(1)
                        .context("⚠️  could move cursor up in terminal")?;
                    term.write_line(&format!(
                        "{} the corpus ({} -> {} files)             ",
                    ...
                    fs::remove_dir_all(&parsed_corpus)
                        .context(format!("⚠️  could not remove dir {}", &parsed_corpus))?;
                    fs::rename(minimized_corpus.clone(), &parsed_corpus).context(format!(
                        "⚠️  could not rename {} to {}",
                        minimized_corpus, &parsed_corpus
                    ))?;
                }
                Err(_) => {
        ...
        for job_num in 0..afl_jobs {
            ...
            fuzzer_handles.push(
                process::Command::new(cargo.clone())
                    ...
                    .spawn()
                    .context("⚠️  could not spawn afl fuzzer")?,
            )
        }
    ...
    if !args.no_honggfuzz {
        ...
        fuzzer_handles.push(
            process::Command::new("script")
                ...
                .spawn()
                .context("⚠️  could not spawn honggfuzz fuzzer")?,
        );
        println!(
            "{} honggfuzz              ",
...
fn run_inputs(args: &Run) -> Result<(), anyhow::Error> {
    ...
    process::Command::new(format!("./target/runner/debug/{}", args.target))
        ...
        .spawn()
        .context("⚠️  could not spawn runner")?
        .wait()
        .context("⚠️  could not wait for runner to finish")?;
...
fn minimize_corpus(
    ...
    process::Command::new(cargo)
        ...
        .spawn()
        .context("⚠️  could not spawn minimizer")?
        .wait()
        .context("⚠️  could not wait for minimizer to finish")?;
...
fn generate_coverage(
    ...
    process::Command::new(cargo)
        ...
        .spawn()
        .context("⚠️  could not spawn rustc for coverage")?
        .wait()
        .context("⚠️  could not wait for rustc for coverage to finish")?;
    ...
    let res = process::Command::new(format!("./target/coverage/debug/{target}"))
        .args([corpus
            .display()
            .to_string()
            .replace("{target_name}", target)])
    ...
    if res.is_ok() {
        if res.as_ref().unwrap().success() {
            ...
            println!(
                "the file {} did not run successfully, errorcode given back is {:?}",
                target,
                res.unwrap().code()
            );
        }
        ...
        return Err(anyhow!(
            "⚠️  could not wait for rustc for coverage to finish {:?}",
            res
        ));
    }
    ...
    process::Command::new("grcov")
        ...
        .spawn()
        .context("⚠️  could not spawn grcov")?
        .wait()
        .context("⚠️  could not wait for grcov to finish")?;
...
fn generate_plot(target: &str, input: &String, output: &Path) -> Result<(), anyhow::Error> {
    ...
    process::Command::new(cargo)
        ...
        .spawn()
        .context("⚠️  could not spawn afl plot")?
        .wait()
        .context("⚠️  could not wait for afl plot o finish")?;
