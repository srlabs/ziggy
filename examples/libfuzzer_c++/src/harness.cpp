...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
  ...
  /// Testsif coverage guidance works
  if (input_str.size() >= 4 && input_str[0] == 'f' && input_str[1] == 'u' &&
      input_str[2] == 'z' && input_str[3] == 'z') {
    std::cout << "That was the good input! Crashing now." << std::endl;
    abort();
  }

  /// Test if ASAN works
  std::string target_str = "asan_crash";
  if (input_str.size() >= target_str.size() &&
      input_str.substr(0, target_str.size()) == target_str) {

    std::cout << "Triggering asan! Crashing now." << input_str << std::endl;
